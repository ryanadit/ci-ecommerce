...
class M_penjualan extends CI_Model{
	...
	function pemesanan(){
		$hsl=$this->db->query("SELECT d_jual_id,d_jual_nofak,d_jual_barang_id,d_jual_barang_nama,d_jual_barang_satuan,d_jual_barang_harpok,d_jual_barang_harjul,d_jual_qty,d_jual_diskon,d_jual_total,d_nama_jual,d_alamat_jual,jual_tanggal FROM tbl_detail_jual JOIN tbl_jual ON jual_nofak=d_jual_nofak");
		return $hsl;
	}

	function hapus_pemesanan($kode){
		$hsl=$this->db->query("DELETE FROM tbl_detail_jual WHERE d_jual_id='$kode'");
		return $hsl;
	}

	function simpan_penjualan($nofak,$total,$nama_jual,$alamat_jual){
		$idadmin=$this->session->userdata('idadmin');
		// untuk menyimpan data penjualan
		$this->db->query("INSERT INTO tbl_jual (jual_nofak,jual_total,nama_jual,alamat_jual,jual_user_id,jual_keterangan) VALUES ('$nofak','$total','$nama_jual','$alamat_jual','$idadmin','eceran')");
		// untuk mengupdate stok ke tabel tbl_barang
		foreach ($this->cart->contents() as $item) {
			$data=array(
				...
				'd_nama_jual'			=>	$nama_jual,
				'd_alamat_jual'			=>	$alamat_jual
			);
